...
class viewEdibles : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_view_edibles)
        ...
        if (userId != null) {
            submissionsRef = database.reference.child("users").child(userId)
                .child("categories").child("submissions").child("Edibles")
        }

        // Read the Edibles submission data from Firebase Realtime Database
        readEdiblesSubmissionFromDatabase()


    }

    private fun readEdiblesSubmissionFromDatabase() {
        val ediblesSubmissionListener = object : ValueEventListener {
        ...
        submissionsRef.addValueEventListener(ediblesSubmissionListener)
    }
